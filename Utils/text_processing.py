...
class Text_Processing:
    ...
    def modified_label_llm(
        self, label_llm, index_llm, now_fieldname, modified_fieldname
    ):
        # Change now to modifed
        label_llm[index_llm] = re.sub(
            now_fieldname, modified_fieldname, label_llm[index_llm]
        )
        ...
        while (index_llm < len(label_llm) - 1) and now_fieldname in label_llm[
            index_llm + 1
        ]:
            # Do something
            label_llm[index_llm + 1] = re.sub(
                now_fieldname, modified_fieldname, label_llm[index_llm + 1]
            )
            index_llm += 1
    ...
    def standard_tagname(self, tagname):
        ...
        tagname = re.sub(
            r"user(\d+)_village", r"user\1_current_address_village", tagname
        )
        tagname = re.sub(r"user(\d+)_ward", r"user\1_current_address_ward", tagname)
        tagname = re.sub(
            r"user(\d+)_district", r"user\1_current_address_district", tagname
        )
        tagname = re.sub(
            r"user(\d+)_province", r"user\1_current_address_province", tagname
        )
    ...
    def get_modifed_tagname(self, contextual, label_llm, index_llm, list_prev_tag=None):
        ...
        if userX is None or value_tagname is None:
            ...
            try:
                ...
                while temp_index >= 0:
                    temp_userX, temp_value_tagname = self.extract_user_and_tag(
                        list_prev_tag[temp_index]
                    )
        ...
        if "_number" in value_tagname:
            ...
            if all(
                temp not in sentence_contextual
                for temp_list in temp_contextual_lists
                ...
                temp in sentence_contextual for temp in list_contextual_health_number
            ):
                new_tagname = f"[{userX}_health_insurance_number]"
            elif any(
                temp in sentence_contextual for temp in list_contextual_social_number
            ):
            ...
            if "last_name" in value_tagname:
                ...
                if "ký" in sentence_contextual or "(ký" in sentence_contextual:
                    ...
                    "tên gọi khác" in sentence_contextual
                    or "tên khác" in sentence_contextual
                ):
        ...
        # Check Birthplace - birth_registration group
            ...
            value_tagname in group_birthplace_tagname
            or value_tagname in group_birth_registration_tagname
        ):
            """
            ...
            """
            if (
                "nơi sinh" in sentence_contextual
                and "birth_registration_place" in tagname
            ):
                new_tagname = re.sub("birth_registration_place", "birthplace", tagname)
                label_llm = self.modified_label_llm(
                    label_llm, index_llm, "birth_registration_place", "birthplace"
                )
            elif (
                "đăng ký" in sentence_contextual or "làm" in sentence_contextual
            ) and "birthplace" in tagname:
                new_tagname = re.sub("birthplace", "birth_registration_place", tagname)
                label_llm = self.modified_label_llm(
                    label_llm, index_llm, "birthplace", "birth_registration_place"
                )

            # return new_tagname
        # # Check id - passport group
        elif (
            value_tagname in group_id_tagname or value_tagname in group_passport_tagname
        ):
            """
            ...
            if any(
                temp in sentence_contextual for temp in list_contextual_id_number
            ) and any(temp in sentence_contextual for temp in list_contextual_passport):
                pass
            elif any(temp in sentence_contextual for temp in list_contextual_id_number):
                if "passport" in tagname:
                    new_tagname = re.sub("passport", "id_number", tagname)
                    label_llm = self.modified_label_llm(
                        label_llm, index_llm, "passport", "id_number"
                    )
            elif any(temp in sentence_contextual for temp in list_contextual_passport):
                if "id" in tagname:
                    new_tagname = re.sub("id", "passport", tagname)
                    label_llm = self.modified_label_llm(
                        label_llm, index_llm, "id", "passport"
                    )

            # Second, check suffix _date, _place
            if "_place" in new_tagname:
                if (
                    "ngày cấp" in sentence_contextual
                    or "cấp ngày" in sentence_contextual
                ):
            ...
            if "_number" in new_tagname:
                if (
                    ...
                    "nơi cấp" in sentence_contextual or "cấp nơi" in sentence_contextual
                ):
                    new_tagname = re.sub("_number", "_issue_place", tagname)
            elif "_date" in new_tagname:
                if (
                    "số" in sentence_contextual
                    and "ngày cấp" not in sentence_contextual
                    and "cấp ngày" not in sentence_contextual
                ):
                    new_tagname = re.sub("_issue_date", "_number", tagname)
            elif "_place" in new_tagname:
                if (
                    "số" in sentence_contextual
                    and "nơi cấp" not in sentence_contextual
                    and "cấp nơi" not in sentence_contextual
                ):
        ...
        # Check dob group
            ...
            if "chữ" in sentence_contextual and "_dob" in tagname:
                ...
                if (
                    "năm" in sentence_contextual
                    and "tháng" not in sentence_contextual
                    and "ngày" not in sentence_contextual
                ):
                    new_tagname = f"[{userX}_dob_year]"
                elif all(
                    temp in sentence_contextual for temp in ["ngày", "tháng", "năm"]
                ):
                ...
                if (
                    "tháng" in sentence_contextual
                    and "ngày" not in sentence_contextual
                    and "năm" not in sentence_contextual
                ):
        ...
        ## With hometown
            ...
            if any(temp in sentence_contextual for temp in list_permanent_address):
                ...
                label_llm = self.modified_label_llm(
                    label_llm, index_llm, "hometown", "permanent_address"
                )
            if any(temp in sentence_contextual for temp in list_current_address):
                new_tagname = re.sub("_hometown", "_current_address", tagname)
                label_llm = self.modified_label_llm(
                    label_llm, index_llm, "hometown", "current_address"
                )
            # return new_tagname
        ## With permanent and current (check if these are mixed)
        elif (
            value_tagname in group_permanent_address_tagname
            or value_tagname in group_current_address_tagname
        ):
            """
            ...
            if (
                "tình trạng" in sentence_contextual
                or "trạng thái" in sentence_contextual
            ) and ("địa chỉ" not in sentence_contextual):
                return "[#another]"
            ...
            if any(temp in sentence_contextual for temp in list_hometown):
                if "_permanent_address" in tagname:
                    ...
                    label_llm = self.modified_label_llm(
                        label_llm, index_llm, "permanent_address", "hometown"
                    )
                elif "_current_address" in tagname:
                    new_tagname = re.sub("_current_address", "_hometown", tagname)
                    label_llm = self.modified_label_llm(
                        label_llm, index_llm, "current_address", "hometown"
                    )
                return new_tagname
            # check right field current, permanent
            elif any(
                temp in sentence_contextual for temp in list_current_address
            ) and any(temp in sentence_contextual for temp in list_permanent_address):
                pass
            elif any(temp in sentence_contextual for temp in list_current_address):
                if "_permanent_address" in tagname:
                    new_tagname = re.sub(
                        "_permanent_address", "_current_address", tagname
                    )
                    label_llm = self.modified_label_llm(
                        label_llm, index_llm, "permanent_address", "current_address"
                    )
            elif any(temp in sentence_contextual for temp in list_permanent_address):
                if "_current_address" in tagname:
                    new_tagname = re.sub(
                        "_current_address", "_permanent_address", tagname
                    )
                    label_llm = self.modified_label_llm(
                        label_llm, index_llm, "current_address", "permanent_address"
                    )
            # return new_tagname
        # Check marial_status
        ...
        # Check social insurance
            ...
            if "social_insurance_number" in value_tagname:
                if any(
                    temp in sentence_contextual
                    for temp in list_contextual_health_number
                ):
                    new_tagname = re.sub(
                        "social_insurance_number", "health_insurance_number", tagname
                    )
            if "health_insurance_number" in value_tagname:
                if any(
                    temp in sentence_contextual
                    for temp in list_contextual_social_number
                ):
                    new_tagname = re.sub(
                        "health_insurance_number", "social_insurance_number", tagname
                    )
        ...
        if value_tagname in group_tagname_have_ward_district_province:
            if (
                any(temp in sentence_contextual for temp in list_village)
                and "village" not in new_tagname
                and all(temp not in sentence_contextual for temp in list_not_village)
            ):
                new_tagname = re.sub("]", "_village]", new_tagname)
            elif (
                any(temp in sentence_contextual for temp in list_ward)
                and "ward" not in new_tagname
                and all(temp not in sentence_contextual for temp in list_not_ward)
            ):
                new_tagname = re.sub("]", "_ward]", new_tagname)
            elif (
                any(temp in sentence_contextual for temp in list_district)
                and "district" not in new_tagname
                and all(temp not in sentence_contextual for temp in list_not_district)
            ):
                new_tagname = re.sub("]", "_district]", new_tagname)
            elif (
                any(temp in sentence_contextual for temp in list_province)
                and "province" not in new_tagname
                and all(temp not in sentence_contextual for temp in list_not_province)
            ):
    ...
    def modify_tagname_dob_date_day_month_year(
        self,
        contextual_input,
        copy_contextual_input,
        label_input,
        index_filled_input,
        contextual_llm,
        label_llm,
        ...
        try:
            ...
            if (
                pattern_dob.search(label_input[index_filled_input])
                and index_filled_input < len(contextual_input) - 1
            ):  # Check tháng năm ở phía sau
                # Kiểm tra có tháng, năm phía sau không (nếu có thì biến đổi thành day, month, year), nếu không thì giữ nguyên
                if (
                    "tháng" in contextual_input[index_filled_input + 1]
                    and "ngày" not in contextual_input[index_filled_input + 1]
                ) or "/" in contextual_input[index_filled_input + 1]:
                    # print(f"debug1.1 at {contextual_input[index_filled_input + 1]}")
                    label_input[index_filled_input + 2] = (
                        f"{label_input[index_filled_input][:-1]}_year]"
                    )
                    label_input[index_filled_input + 1] = (
                        f"{label_input[index_filled_input][:-1]}_month]"
                    )
                    label_input[index_filled_input] = (
                        f"{label_input[index_filled_input][:-1]}_day]"
                    )
                    copy_contextual_input[index_filled_input] = (
                        contextual_llm[index_llm] + [":"] + [label_llm[index_llm]]
                    )
                    # Điền thêm 2 nên phải + = 2
                    if index_llm < len(contextual_llm) - 1:
                        if (
                            "tháng" in contextual_llm[index_llm + 1]
                            and "ngày" not in contextual_llm[index_llm + 1]
                        ) or "/" in contextual_llm[index_llm + 1]:
                            copy_contextual_input[index_filled_input + 1] = (
                                contextual_llm[index_llm + 1]
                                + [":"]
                                + [label_llm[index_llm + 1]]
                            )
                            copy_contextual_input[index_filled_input + 2] = (
                                contextual_llm[index_llm + 2]
                                + [":"]
                                + [label_llm[index_llm + 2]]
                            )
            ...
            if (
                pattern_date.search(label_input[index_filled_input])
                and index_filled_input < len(contextual_input) - 1
            ):
            ...
            if pattern_day.search(
                label_input[index_filled_input]
            ) and index_filled_input < len(contextual_input):
                ...
                if (
                    (index_filled_input == len(contextual_input) - 1)
                    or (
                        "tháng" not in contextual_input[index_filled_input + 1]
                        and "/" not in contextual_input[index_filled_input + 1]
                    )
                    or (
                        "ngày" in contextual_input[index_filled_input + 1]
                        and "tháng" in contextual_input[index_filled_input + 1]
                    )
                ):
                    ...
                    if (
                        index_llm < len(contextual_llm) - 1
                        and f"{prefix_day}_month]" in label_llm[index_llm + 1]
                    ):
                        index_llm += 2
                elif (
                    index_filled_input < len(contextual_input) - 3
                ):  # có thể điền hai chỗ nữa
                    if (
                        "tháng" in contextual_input[index_filled_input + 1]
                        and "ngày" not in contextual_input[index_filled_input + 1]
                    ) or "/" in contextual_input[index_filled_input + 1]:
    ...
    def check_prev_now_next_context_similiar(
        self,
        contextual_input,
        label_input,
        index_input,
        ...
        if index_input > 0:
            prev_name_contextual_input = self.get_hash_name_from_context_at_index(
                contextual_input, index_input - 1
            )
        if index_input < len(contextual_input) - 1:
            next_name_contextual_input = self.get_hash_name_from_context_at_index(
                contextual_input, index_input + 1
            )
        # Get prev and next contextual llm
        if index_llm > 0:
            prev_name_contextual_llm = self.get_hash_name_from_context_at_index(
                contextual_llm, index_llm - 1
            )
        if index_llm < len(contextual_llm) - 1:
            next_name_contextual_llm = self.get_hash_name_from_context_at_index(
                contextual_llm, index_llm + 1
            )
        # Get context now
        name_contextual_input = self.get_hash_name_from_context_at_index(
            contextual_input, index_input
        )
        name_contextual_llm = self.get_hash_name_from_context_at_index(
            contextual_llm, index_llm
        )
        # Checking
        if name_contextual_input != name_contextual_llm:
            return False
        # Tagname hiện tại là tháng/năm/\/
        if len(contextual_input[index_input]) == 1 and any(
            temp in name_contextual_input for temp in ["tháng", "năm", "/"]
        ):
            if (
                prev_name_contextual_input is not None
                and prev_name_contextual_llm is not None
                and prev_name_contextual_input != prev_name_contextual_llm
            ):
                return False
            if (
                next_name_contextual_input is not None
                and next_name_contextual_llm is not None
                and next_name_contextual_input != next_name_contextual_llm
            ):
                return False
    ...
    def get_tagnames_from_LLM_filled_form(
        ...
        index_llm = 0
        while index_filled_input < len(contextual_input) and index_llm < len(
            contextual_llm
        ):
            # print(f"Context input: {contextual_input[index_filled_input]} - llm:  {contextual_llm[index_llm]}")
            if self.check_prev_now_next_context_similiar(
                contextual_input,
                label_input,
                index_filled_input,
                contextual_llm,
                ...
                if not process_tagname:
                    ...
                    tagname_fill = self.get_modifed_tagname(
                        contextual_input[index_filled_input],
                        label_llm,
                        index_llm,
                        label_input[:index_filled_input],
                    )
                ...
                T = True
                while (
                    (temp_count < len(contextual_input) - index_filled_input)
                    or (temp_count < len(contextual_llm) - index_llm)
                ) and (temp_count < 10):
                    """
                    Print for debugging
                    ...
                    if (temp_count < len(contextual_input) - index_filled_input) and (
                        (index_filled_input + temp_count) < len(contextual_input)
                    ):
                        if self.check_prev_now_next_context_similiar(
                            contextual_input,
                            label_input,
                            index_filled_input + temp_count,
                            contextual_llm,
                            label_llm,
                            index_llm,
                        ):
                    ...
                    if (temp_count < len(contextual_llm) - index_llm) and (
                        (index_llm + temp_count) < len(contextual_llm)
                    ):
                        if self.check_prev_now_next_context_similiar(
                            contextual_input,
                            label_input,
                ...
                if T:
                    ...
                    try:
                        print(
                            f"Index {index_filled_input} with {len(contextual_input)} context {contextual_input[index_filled_input]} t {label_input[index_filled_input]}"
                        )
                        print(
                            f"Index {index_llm} with {len(contextual_llm)} context {contextual_llm[index_llm]} t {label_llm[index_llm]}"
                        )
                        print()
                    except Exception as e:
            ...
            (
                ...
                label_llm,
                index_llm,
            ) = self.modify_tagname_dob_date_day_month_year(
                contextual_input,
                copy_contextual_input,
                label_input,
                index_filled_input,
    ...
    def fill_input_by_llm_form(self, form_llm_filled, input_form, process_tagname=True):
        ...
        # List tagname
        tagname_for_input, copy_contextual_input = (
            self.get_tagnames_from_LLM_filled_form(
                contextual_llm,
                label_llm,
                contextual_input,
                label_input,
    ...
    def get_tagnames_from_label_form(
        ...
        while index_filled_input < len(contextual_input) and index_llm < len(
            ...
            name_contextual_llm = self.get_hash_name_from_context_at_index(
                contextual_label, index_llm
            )
            if name_contextual_input == name_contextual_llm:
                # label_input[index_filled_input] = label_llm[index_llm]
                tagname_fill = self.get_modifed_label_tagname(
                    contextual_input[: index_filled_input + 1], label_label[index_llm]
                )
                label_input[index_filled_input] = tagname_fill

                copy_contextual_input[index_filled_input] = (
                    contextual_label[index_llm] + [":"] + [label_label[index_llm]]
                )
            else:
    ...
    def convert_label_form_to_input_form(self, label_folder, input_folder):
        ...
        for index, filename in enumerate(os.listdir(label_folder)):
            if filename.endswith(".txt"):
                ...
                print(
                    f"Save successfully file {filename} at index {index} at {input_path}"
                )
