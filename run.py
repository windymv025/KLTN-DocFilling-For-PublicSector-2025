    ...
    for index,filename in enumerate(os.listdir(input_folder)):
        if filename.endswith(".txt"):
            ...
            try:
                ...
                filled_input_text = Text_Processing().fill_input_by_llm_form(filled_text, input_text)
                # Remove different tagnames
                filled_input_text_different = Text_Processing().remove_different_tagnames(filled_input_text)
                #Save 2 versions
                output_path = output_folder + '/' + filename
                output_path_different = output_folder + '/Differents/' + filename
                Text_Processing().Save_txt_file(output_path, filled_input_text)
                Text_Processing().Save_txt_file(output_path_different, filled_input_text_different)
...
for index,filename in enumerate(os.listdir(label_by_hand_folder)):
    if filename.endswith(".txt"):
        ...
        try:
            # Remove different tagnames
            label_text_different = Text_Processing().remove_different_tagnames(label_text)
            #Save
